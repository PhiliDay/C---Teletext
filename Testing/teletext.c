    ...
    for(j = 0; j < WIDTH; j++){
      counter = fread(&ceefax[i][j].teletext, sizeof(unsigned char), 1, filePointer);
        ...
        /*Sets all the control codes to less than 128*/
         ...
         if(cnt == 0){
           printf("Test 1 failed - Values incorrect\n");
         }
    }
  }
...
void printCeefax(Code ceefax[LENGTH][WIDTH])
{
  ...
  readFont(fontdata, "m7fixed.fnt");
   ...
   for(i=0;i<LENGTH;i++){
      for(j=0;j<WIDTH;j++){
       ...
       if(ceefax[i][j].teletext == HOLD){
        ...
        if(holdFlag == ON){
          ...
          assert(holdValue.background == ceefax[i][j].teletext-GOTOCOLOUR);
        }
        ...
        if(ceefax[i][j].Graphics == Graphics1){
          assert(ceefax[i][j].Graphics == Graphics1);
     ...
     /*Testing*/
     if(ceefax[24][5].Graphics != Graphics1 && ceefax[24][5].foreground != MAGENTA){
       cnt++;
     }
     if(ceefax[18][13].Graphics != Graphics1 && ceefax[18][13].Type != Seperated){
       cnt++;
     }
     if(cnt == 2){
         printf("Incorrect Test 2 with test.m7 file\n");
      }
...
void createCeefax(Code ceefax[LENGTH][WIDTH])
{
   ...
   for(i=0;i<LENGTH;i++){
      for(j=0;j<WIDTH;j++){
         ...
         (assert(ceefax[i][j].sixel.botl == OFF));
         (assert(ceefax[i][j].sixel.botr == OFF));
         (assert(ceefax[i][j].sixel.topl == OFF));
         (assert(ceefax[i][j].sixel.topr == OFF));
         (assert(ceefax[i][j].sixel.midl == OFF));
         (assert(ceefax[i][j].sixel.midr == OFF));

      /*Making the default line settings */
      if(j == 0){
         ceefax[i][j] = newLine(ceefax[i][j], &before);
         (assert(ceefax[i][j].background == BLACK));
         (assert(ceefax[i][j].foreground == WHITE));
         (assert(ceefax[i][j].Height == Single));
         (assert(ceefax[i][j].Graphics == Alphanumeric));
         (assert(ceefax[i][j].Type == Contiguous));
         (assert(ceefax[i][j].Held == Unheld));
      }
   ...
   /*Testing*/
   if(ceefax[0][4].teletext == SPACE){
     cnt++;
   }
   if(ceefax[0][9].teletext == 'E'){
     cnt++;
   }
   if(ceefax[2][12].Height == DOUBLE){
     cnt++;
   }

   if(cnt == 0){
       printf("Not correct amount of values - TEST 3 FAILED \n");
     }
}
...
Code newLine(Code c, Code *before)
{
  ...
  /*Creating default settings for a new line*/
   ...
   /*Testing*/
   if(c.background == BLACK || c.foreground == WHITE || c.Height == SINGLE
   || c.Graphics == Alphanumeric || c.Type == Contiguous || c.Held == Unheld){
     cnt++;
   }
   if(cnt == 0){
       printf("Not correct amount of values - TEST 4 FAILED \n");
     }
...
Code makeGraphics(Code c, int code)
{
   ...
   c.sixel.topl = teletext / TOPL;
    ...
    /*Testing 4*/
    if(c.Graphics == Alphanumeric){
    if(c.sixel.botr == 0 && c.sixel.botl == 0 && c.sixel.midr == 0 &&
      c.sixel.midl == 0 && c.sixel.topr == 0 && c.sixel.topl == 0){
      cnt++;
      }
    }
    if(cnt != 0){
        printf("Not correct amount of values - TEST 5 FAILED \n");
      }
...
int cnt = 0;
  ...
  if(ceefax[i][j].Height == OFF){
      ...
      /*Testing - DoubleHeight*/
      if(ceefax[2][13].topBottom == TOP && ceefax[2][12].Height == DOUBLE){
        cnt++;
      }

      if(cnt == 1){
          printf("Test 5 Failed - Height incorrect\n");
        }
...
{
   if(c.teletext >= RED && c.teletext <= WHITE){
      assert(c.teletext >= RED && c.teletext <= WHITE);
      c.foreground = c.teletext;
      assert(c.foreground == c.teletext);

   }
   else if(c.teletext >= REDG && c.teletext <= WHITEG){
     assert(c.teletext >= REDG && c.teletext <= WHITEG);
